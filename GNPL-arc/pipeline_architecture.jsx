...
const EDGES = [
  ...
  { id: "E8",  from: "J2",   to: "J6",   type: "valve" },
  ...
  { id: "E14", from: "J7",   to: "STO",  type: "valve" },
  { id: "E15", from: "STO",  to: "J5",   type: "valve" },
...
const NOMINAL_PRESSURES = {
  S1:"50 bar",J1:"50 bar",CS1:"50 bar",J2:"50 bar",J3:"50 bar",J4:"50 bar",
  CS2:"50 bar",J5:"50 bar",J6:"48 bar",PRS1:"30 bar",J7:"50 bar",STO:"50 bar",
  PRS2:"25 bar",S2:"48 bar",D1:"30 bar",D2:"30 bar",D3:"25 bar",D4:"25 bar",D5:"45 bar",D6:"48 bar",
};

const INFO = {
  S1:   { title:"Source S1",           color:"#15803d", body:"Primary gas source. Pressure driven by generateSourceProfile.m — AR(1) random walk + multi-frequency oscillations. Init: 50 bar. Attack A1 manipulates this node." },
  S2:   { title:"Source S2",           color:"#15803d", body:"Secondary gas source (node 14). Feeds J7 via edge E13. Init: 48 bar. Provides redundancy for the eastern distribution network." },
  J1:   { title:"Junction J1",         color:"#475569", body:"Pre-compressor junction (node 2). Connects source S1 to CS1 inlet via E1→E2. Nodal volume 6 m³ (lumped capacitance model)." },
  CS1:  { title:"Compressor CS1",      color:"#6d28d9", body:"Primary compressor (node 3). CS1 PID maintains p_D1 at 30 bar setpoint. Head: H = 800 − 0.8ṁ − 0.002ṁ². Efficiency: η = 0.82 − 0.002ṁ. Ratio range 1.05–1.80. cs1_alarm fires at ratio ≥ 1.75." },
  J2:   { title:"Junction J2 — Branch",color:"#475569", body:"Main branch junction (node 4). Splits flow into: main trunk E4→J3, and side branch via valve E8→J6→PRS1. Incidence matrix B(20×20) governs mass balance." },
  J3:   { title:"Junction J3",         color:"#475569", body:"Mid-network junction (node 5). Receives flow from J2 via E4 and connects to J7 via E12. Attack A8 targets edge E12 (pipeline leak)." },
  J4:   { title:"Junction J4",         color:"#475569", body:"Junction before CS2 (node 6). Also feeds demand D5 via E19 (vertical bypass). Connects to CS2 via E6." },
  CS2:  { title:"Compressor CS2",      color:"#6d28d9", body:"Secondary compressor (node 7). CS2 PID maintains p_D3 at 25 bar setpoint. Head: H = 500 − 0.5ṁ − 0.001ṁ². Ratio range 1.02–1.60. cs2_alarm fires at ratio ≥ 1.55." },
  J5:   { title:"Junction J5",         color:"#475569", body:"Eastern distribution junction (node 8). Receives output from CS2 (E7) and optionally from storage cavern STO via valve E15. Feeds PRS2 via E16." },
  J6:   { title:"Junction J6",         color:"#475569", body:"Side branch junction (node 9). Fed from J2 via valve E8. Feeds PRS1 via E9. Opens when p_J6 < 28 bar, closes when p_J6 > 55 bar." },
  PRS1: { title:"PRS1 — 30 bar",       color:"#047857", body:"Pressure Regulating Station (node 10). Downstream setpoint 30 bar. First-order throttle response τ = 5s, deadband ±0.5 bar. Feeds D1 (E10) and D2 (E11)." },
  J7:   { title:"Junction J7",         color:"#475569", body:"Upper mid-network junction (node 11). Receives flow from J3 (E12) and second source S2 (E13). Connects to storage STO via valve E14 and to demand D6 via E20." },
  STO:  { title:"Storage Cavern STO",  color:"#1d4ed8", body:"Underground storage node (node 12). Bidirectional: inject (E14) when p_J7 > 52 bar, withdraw (E15) when p_J5 < 46 bar. Inventory tracked as fraction 0–1." },
  PRS2: { title:"PRS2 — 25 bar",       color:"#047857", body:"Pressure Regulating Station (node 13). Downstream setpoint 25 bar. Feeds D3 (E17) and D4 (E18). Behaviour same as PRS1: first-order τ = 5s." },
  D1:   { title:"Demand D1",           color:"#b91c1c", body:"Primary delivery node (node 15). CS1 PID target: 30 bar. EKF tracks this node for CS1 control feedback. Attack A5 spoofs the pressure sensor at this node." },
  D2:   { title:"Demand D2",           color:"#b91c1c", body:"Secondary delivery via PRS1 (node 16). Receives flow from PRS1 via E11." },
  D3:   { title:"Demand D3",           color:"#b91c1c", body:"Eastern delivery node (node 17). CS2 PID target: 25 bar. Attack A6 spoofs flow meter on feeding edges." },
  D4:   { title:"Demand D4",           color:"#b91c1c", body:"Eastern delivery node (node 18). Receives flow from PRS2 via E18." },
  D5:   { title:"Demand D5",           color:"#b91c1c", body:"Direct bypass demand (node 19). Receives flow from J4 via E19 without PRS throttling." },
  D6:   { title:"Demand D6",           color:"#b91c1c", body:"Upper network demand (node 20). Receives flow from J7 via E20. Attack A4 manipulates demand at this type of node." },
  E8:   { title:"Valve E8: J2 → J6",  color:"#1d4ed8", body:"Side branch isolation valve. Controlled by PLC: open (1000) when p_J6 < 28 bar, close (0) when p_J6 > 55 bar. Command delivered via Modbus register 102. Attack A3 can force this valve closed." },
  E14:  { title:"Valve E14: J7 → STO (inject)",   color:"#1d4ed8", body:"Storage injection valve. Opens when p_J7 > 52 bar. Coil 3 (sto_inject_active) set when active." },
  E15:  { title:"Valve E15: STO → J5 (withdraw)", color:"#1d4ed8", body:"Storage withdrawal valve. Opens when p_J5 < 46 bar. Coil 4 (sto_withdraw_active) set when active." },
};
...
function ValveSymbol({ from, to, id, selected, onClick }) {
  ...
  return (
    <g transform={`translate(${mid.x},${mid.y}) rotate(${deg})`} onClick={onClick} style={{ cursor: "pointer" }}>
      <line x1={-30} y1={0} x2={30} y2={0} stroke={isSelected ? "#b45309" : "#94a3b8"} strokeWidth={3.5} />
      ...
      <rect x={-12} y={-10} width={24} height={20} rx={2} fill="none" stroke={col} strokeWidth={1.2} opacity={0.4} />
    </g>
  );
}
  ...
  return (
    <g transform={`translate(${node.x},${node.y})`} onClick={onClick} style={{ cursor: "pointer" }}>
      <circle cx={0} cy={0} r={20}
        fill={isSelected ? "rgba(180,83,9,0.12)" : "rgba(109,40,217,0.1)"}
...
function PRSSymbol({ node, id, selected, onClick }) {
  const isSelected = selected === id;
  const col = isSelected ? "#b45309" : "#047857";
  return (
    <g transform={`translate(${node.x},${node.y})`} onClick={onClick} style={{ cursor: "pointer" }}>
      <rect x={-16} y={-12} width={32} height={24} rx={4}
        fill={isSelected ? "rgba(180,83,9,0.12)" : "rgba(4,120,87,0.1)"}
  ...
  return (
    <g transform={`translate(${node.x},${node.y})`} onClick={onClick} style={{ cursor: "pointer" }}>
      <ellipse cx={0} cy={0} rx={20} ry={12}
        fill={isSelected ? "rgba(180,83,9,0.12)" : "rgba(29,78,216,0.1)"}
        stroke={col} strokeWidth={2}
        style={{ filter: isSelected ? `drop-shadow(0 0 5px ${col})` : "none", transition: "all 0.2s" }} />
      <rect x={-20} y={-12} width={40} height={18}
        fill={isSelected ? "rgba(180,83,9,0.06)" : "rgba(29,78,216,0.06)"} stroke="none" />
  ...
  return (
    <g transform={`translate(${node.x},${node.y})`} onClick={onClick} style={{ cursor: "pointer" }}>
      ...
      <text x={0} y={r + 13} textAnchor="middle" fontSize={10} fontWeight="800"
        fill={isSelected ? "#b45309" : col}
        fontFamily="'Fira Code', monospace" style={{ pointerEvents: "none" }}>
        {node.label}
      </text>
    </g>
  );
...
export default function App() {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding: "18px 24px 0", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #e2e8f0", paddingBottom: 14 }}>
        <div>
          <div style={{ fontSize: 10, color: "#0369a1", fontFamily: "'Fira Code', monospace", letterSpacing: 3, textTransform: "uppercase" }}>
            Physical Network Topology
          </div>
          <h1 style={{ margin: "3px 0 0", fontSize: 19, fontWeight: 800, color: "#0f172a", letterSpacing: -0.5 }}>
            20-Node Gas Transmission Pipeline
          </h1>
        </div>
        <div style={{ display: "flex", gap: 16, fontSize: 10, color: "#64748b", flexWrap: "wrap", maxWidth: 480 }}>
          {[
            ["●","#15803d","Source (2)"],["●","#475569","Junction (7)"],["▲","#b91c1c","Demand (6)"],
            ["◎","#6d28d9","Compressor (2)"],["▬","#047857","PRS (2)"],["⌀","#1d4ed8","Storage (1)"],["⋈","#1d4ed8","Valve (3)"],
          ].map(([sym, col, lbl]) => (
      ...
      <div style={{ display: "flex", flex: 1, padding: "12px 16px 16px", gap: 14, minHeight: 560 }}>
        ...
        <div style={{
          ...
          border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden", position: "relative",
          ...
          <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" style={{ position: "relative", zIndex: 1 }}>
            ...
            <text x={420} y={235} textAnchor="middle" fontSize={8.5} fill="#94a3b8" fontFamily="sans-serif">main trunk</text>
            <text x={420} y={385} textAnchor="middle" fontSize={8.5} fill="#94a3b8" fontFamily="sans-serif">side branch (valve)</text>
            <text x={590} y={115} textAnchor="middle" fontSize={8.5} fill="#94a3b8" fontFamily="sans-serif">storage loop</text>
            ...
            {pipes.map(e => {
              ...
              return (
                <g key={e.id}>
                  {isSel && <line x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#b45309" strokeWidth={9} opacity={0.1} strokeLinecap="round" />}
            ...
            {["S1","J2","J3","J4","J5","PRS1","PRS2","D1","D3"].map(id => {
              const n = NODES[id];
              return (
                <circle key={`ekf-${id}`} cx={n.x + 14} cy={n.y - 14} r={3.5}
                  fill="rgba(180,83,9,0.4)" stroke="#b45309" strokeWidth={0.8}
                  style={{ pointerEvents: "none" }} />
              );
            })}
            <text x={55} y={230} fontSize={7.5} fill="#b45309" fontFamily="'Fira Code', monospace" style={{ pointerEvents: "none" }}>● EKF-40</text>
          </svg>
        </div>
        ...
        <div style={{
          ...
          {info ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
              <div style={{ color: "#94a3b8", fontSize: 9, fontFamily: "'Fira Code', monospace", letterSpacing: 1 }}>
                CLICK NODE / EDGE TO INSPECT
              </div>
              {[
                ["Nodes","20","#15803d"],["Edges","20","#0369a1"],["Sources","2","#15803d"],
                ["Compressors","CS1 + CS2","#6d28d9"],["PRS stations","PRS1 + PRS2","#047857"],
                ["Storage","1 cavern","#1d4ed8"],["Control valves","E8 E14 E15","#1d4ed8"],
                ["Demand nodes","D1–D6","#b91c1c"],["EKF state dim","40 (20p+20q)","#b45309"],
                ["Modbus regs","70 + 7 coils","#0e7490"],["Attack labels","A1–A10","#b91c1c"],
              ].map(([lbl, val, col]) => (
              ...
              <div style={{ marginTop: "auto", borderTop: "1px solid #f1f5f9", paddingTop: 12 }}>
                <div style={{ color: "#94a3b8", fontSize: 9, fontFamily: "'Fira Code', monospace", letterSpacing: 1, marginBottom: 8 }}>
                  PIPE PARAMS (nominal)
                </div>
