...
const EDGES = [
  ...
  { id: "E10", from: "PRS1", to: "D1",   type: "valve" },   // V_D1 isolation valve
  ...
  { id: "E21", from: "J4",   to: "J7",   type: "resilience" },  // cross-tie bypass CS2
  { id: "E22", from: "J3",   to: "J5",   type: "resilience" },  // emergency bypass
];
...
const INFO = {
  ...
  E8:   { title: "Valve E8: J2 → J6",   color: "#60a5fa", body: "Side branch isolation valve. Controlled by PLC: open (1000) when p_J6 < 18 bar, close (0) when p_J6 > 25 bar. Command delivered via Modbus register 102. Attack A3 can force this valve closed." },
  E10:  { title: "Valve E10 (V_D1): PRS1 → D1", color: "#f87171", body: "D1 isolation valve (Phase 7 addition). Modbus addr 109. PLC closes automatically if p_D1 > 20 barg (overpressure protection per PNGRB T4S Annexure III). Reopens with 0.5 bar hysteresis at p_D1 < 19.5 barg." },
  E14:  { title: "Valve E14: J7 → STO (inject)", color: "#60a5fa", body: "Storage injection valve. Opens when p_J7 > 24 bar. Coil 3 (sto_inject_active) set when active. Part of the storage loop E14→STO→E15." },
  E15:  { title: "Valve E15: STO → J5 (withdraw)", color: "#60a5fa", body: "Storage withdrawal valve. Opens when p_J5 < 20 bar. Coil 4 (sto_withdraw_active) set when active. Closes the network loop STO→J5." },
  E21:  { title: "E21: J4 → J7 Cross-Tie (Phase 7)", color: "#f59e0b", body: "Resilience edge — DN100 (0.102 m ID), 8 km, API 5L Gr.B. Activates when CS2 alarm fires (PLC coil cs2_alarm). Provides 40–60% of normal CS2 flow via alternate path. Satisfies PNGRB T4S §6.3 N-1 continuity. Modbus addr 110 (crosstie_E21_cmd). WARNING: also a new attack surface — force-close during CS2 failure causes outage." },
  E22:  { title: "E22: J3 → J5 Emergency Bypass (Phase 7)", color: "#f59e0b", body: "Resilience edge — DN80 (0.082 m ID), 12 km, API 5L Gr.B. Activates only in extreme stress: p_J5 < 18 barg AND p_J4 > 20 barg AND CS2 ratio > 1.40. Maintains minimum 30% flow to D3/D4 during double contingency. PNGRB T4S §5.4. Modbus addr 111 (bypass_E22_cmd)." },
};
...
export default function App() {
  ...
  const resilience = EDGES.filter(e => e.type === "resilience");
  ...
  return (
    <div style={{
          ...
          <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%"
            ...
            {resilience.map(e => {
              ...
              return (
                <g key={e.id}>
                  {isSel && <line x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                    stroke="#f59e0b" strokeWidth={9} opacity={0.15} strokeLinecap="round" />}
                  <line x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                  ...
                  <text
                    ...
                    y={(a.y + b.y) / 2}
                    textAnchor="middle" fontSize={9}
                    fill={isSel ? "#f59e0b" : "rgba(245,158,11,0.55)"}
                    fontFamily="'Fira Code', monospace"
                    style={{ pointerEvents: "none" }}>{e.id}</text>
                </g>
              );
      ...
      <div style={{ textAlign: "center", paddingBottom: 8, color: "rgba(255,255,255,0.1)", fontSize: 10, fontFamily: "'Fira Code', monospace" }}>
        20 nodes · 22 edges (20+E21+E22) · 4 valve edges (E8/E10/E14/E15) · 2 resilience edges · Darcy-Weisbach + Peng-Robinson EOS · EKF state dim = 40
      </div>
    </div>
  );
}
