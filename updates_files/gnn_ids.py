"""
models/gnn_ids.py — Graph Neural Network for CPS Communication Anomaly Detection
==================================================================================
Role in the IDS stack:
  ...
  Detects attacks that change communication patterns:
    - MITM (new communication pair appears)
    - Command injection (SCADA_01 → wrong device)
    - Replay (same comm pair with abnormal frequency)
    - Latency attack (inter_pkt_ms spikes on specific edges)

Two graph inputs are supported:
  1. Physical topology graph  — 20 nodes, 20 edges (pipeline connectivity)
  2. Communication graph      — 23 devices, edges weighted by comm_freq
     (built from network_logger.py output)

Architecture (simplified GDN — Graph Deviation Network):
  1. For each node, learn an attention-weighted embedding from its neighbours
...
This avoids needing labelled data — the model learns normal graph dynamics
and flags deviations, making it suitable for zero-day attack detection.

Usage:
  from models.gnn_ids import GraphAnomalyDetector
  det = GraphAnomalyDetector(node_feat_dim=10, edge_index=adj)
  det.fit(node_feature_sequences)   # shape: (N_timesteps, n_nodes, feat_dim)
...
# Physical pipeline topology — 20-node CGD network
# Format: list of (from_node_idx, to_node_idx), 0-indexed
PIPELINE_EDGES = [
    (0,  1),   # E1:  S1→J1
    (1,  2),   # E2:  J1→CS1
    (2,  3),   # E3:  CS1→J2
...
def build_pipeline_adj(n_nodes: int = 20,
    ...
    """
    Adjacency matrix for the 20-node pipeline physical topology.
    Returns: (n_nodes, n_nodes) float tensor, symmetric if bidirectional.
    """
...
def build_comm_adj_from_df(df, n_devices: int = 23) -> torch.Tensor:
    """
    Build a device communication adjacency matrix from the network logger CSV.
    Edge weight = normalised communication frequency between device pairs.

    Parameters
    ----------
    df        : pandas DataFrame with columns src_id, dst_id (device IDs)
    n_devices : number of devices in registry (23 for the full network)

    Returns: (n_devices, n_devices) float tensor
    """
    # Device ID → integer index mapping (matches device_registry.m order)
    DEVICE_IDX = {
    ...
    # Normalise + self-loops
    adj = adj / (adj.max() + 1e-8)
    adj += torch.eye(n_devices)
    deg = adj.sum(dim=1, keepdim=True).clamp(min=1)
    return adj / deg


# ─────────────────────────────────────────────────────────────────────────────
# GNN layer — attention-weighted graph convolution
# ─────────────────────────────────────────────────────────────────────────────
...
class GraphDeviationNetwork(nn.Module):
    ...
    The anomaly score at node i is |predicted_i − actual_i|.
    """
...
class GraphAnomalyDetector:
    ...
    node_feat_dim : features per node per timestep
                    (e.g. 5: pressure, flow, EKF residual, CUSUM, plc_p)
    n_nodes       : number of nodes in graph (20 for pipeline, 23 for comm graph)
    ...
    def fit(self, X_normal: np.ndarray, epochs: int = 30,
        ...
        Parameters
        ----------
        X_normal : np.ndarray, shape (N, n_nodes, node_feat_dim)
                   Normal-only node feature snapshots.
        """
    ...
    def anomaly_score(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            for (xb,) in loader:
                xb   = xb.to(self.device)
                pred = self.model(xb, self.adj)
                err  = self._loss_fn(pred, xb).mean(dim=(1, 2))
                scores.append(err.cpu().numpy())
        return np.concatenate(scores)

    def node_scores(self, X: np.ndarray) -> np.ndarray:
        """
        Per-node anomaly scores — useful for attack localisation.
        Shape: (N, n_nodes).
        Higher score at node i → that physical location is anomalous.
        """
        self.model.eval()
        all_scores = []
        loader = DataLoader(
        ...
        return np.concatenate(all_scores, axis=0)   # (N, n_nodes)
    ...
    def fit_threshold(self, X_val_normal: np.ndarray,
        ...
        print(f"[gnn] Threshold set at {self.threshold_:.5f} (FPR={fpr:.2f})")
    ...
    for node_idx, (node_name, cols) in enumerate(node_feature_map.items()):
        for feat_idx, col in enumerate(cols):
            if col in df.columns:
                X[:, node_idx, feat_idx] = df[col].fillna(0).values.astype(np.float32)
    return X


# Default node feature map for the 20-node CGD network
# Uses 3 features per node: physics pressure, EKF residual, PLC reading
